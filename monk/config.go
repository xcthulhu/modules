...
func InitChain() error {
	...
	return utils.WriteJson(doug.DefaultGenesis, path.Join(utils.Blockchains, "genesis.json"))
}
...
// Set package global variables (thelutil.Config, logging).
...
func (mod *MonkModule) thConfig() {
	...
	// a global thelutil.Config object is used for shared global access to the db.
	// this also uses rakyl/globalconf, but we mostly ignore all that
	thelutil.Config = &thelutil.ConfigManager{ExecPath: cfg.RootDir, Debug: true, Paranoia: true}
