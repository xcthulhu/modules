...
func (mint *MintModule) WorldState() *types.WorldState {
	...
	state.GetAccounts().Iterate(func(key interface{}, value interface{}) bool {
		hexAddr := hex.EncodeToString(account.Account.Address)
		stateMap.Order = append(stateMap.Order, hexAddr)
		account := &types.Account{
			Address: hexAddr,
			Balance: strconv.FormatUint(account.Account.Balance, 10),
			Nonce:   strconv.FormatUint(account.Account.Sequence, 10),
			// TODO:
			//Script:   script,
			//Storage:  storage,
			//IsScript: isscript,
...
func (eth *Eth) State() *types.State {
	...
	for it.Next() {
		//(func(addr string, acct *ethutil.Value) {
		addr := it.Key
		//acct := it.Value
